...
def add_topic(topic, token, username, reponame):
    if not token:
        print(
            "Error: GitHub Access Token is required. Provide it as an argument or set ADMIN_TOKEN in .env"
        )
        sys.exit(1)
    if not username:
        print(
            "Error: Username is required. Provide it as an argument or ensure git is configured."
        )
        sys.exit(1)
    if not reponame:
    ...
    if response.status_code != 200:
        print(
            f"Error fetching current topics({url}): {response.status_code} {response.text}"
        )
        sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        "--token", help="GitHub Access Token (default: ADMIN_TOKEN from .env)"
    )
    parser.add_argument(
        "--username", help="GitHub username (default: current git username)"
    )
    parser.add_argument(
        "--reponame", help="GitHub repository name (default: current folder name)"
    )
